...
The working tree is in the middle of {{OPERATION}} into the `{{INTEGRATION}}` branch, which was just reset to
`{{PORT}}` (the fork's own changes). The common base is `{{BASE}}` and the incoming side is `{{THEIRS}}`.
These files are conflicted:
...
1. For each conflicted file, understand what the incoming side changed (`git diff {{BASE}} {{THEIRS}} -- <file>`,
   `git log -p {{BASE}}..{{THEIRS}} -- <file>`) and what the port changed (`git diff {{BASE}} HEAD -- <file>`).
2. Edit the file so it keeps **both** intents: the incoming change applied to the port's behavior.
   Remove every conflict marker.
3. Optionally run targeted tests with `{{AGENT_TEST}} <pytest args>`. A failing test is a result to report,
   not something to fix.
4. Write `RESOLUTION.md` at the repository root with, per file: what the incoming side changed, what the port
...
- Do not run `git add`, `git commit`, `git merge`, `git read-tree`, `git reset`, `git checkout`, `git restore`, or `git push`.
